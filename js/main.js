...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // btn-clean vive DENTRO del cam-panel que la vista limpia oculta, así que
  // no sirve para salir una vez activada. btn-exit-clean vive fuera de ese
  // panel a propósito, y Esc es la salida de emergencia sin mouse.
  const btnClean     = document.getElementById('btn-clean');
  const btnExitClean = document.getElementById('btn-exit-clean');

  function _setCleanMode(_forceOff = false) {
    const wasActive = document.body.classList.contains('clean-mode');
    if (_forceOff && !wasActive) return; // nada que hacer
    const clean = toggleCleanView();
    if (btnClean) btnClean.classList.toggle('active', clean);
    _addLog('info', clean ? 'Vista limpia activada' : 'Vista limpia desactivada');
  }

  if (btnClean) btnClean.addEventListener('click', () => _setCleanMode());
  if (btnExitClean) btnExitClean.addEventListener('click', () => _setCleanMode(true));
