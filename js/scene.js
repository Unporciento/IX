...
function _buildTerrain() {
  ...
  // Arena de playa: franja de X=-70 a X=-44, a lo largo de toda la costa (Z)
  const sandGeo = new THREE.PlaneGeometry(26, 260, 24, 80);
  ...
  sand.position.set(-57, 0, 0); // centro de la franja -70..-44
  sand.receiveShadow = true;
  scene.add(sand);

  // Jardín frente a las casas: franja de X=-44 a X=-2 (hasta el borde de la calle)
  const gardenTex = _makeSandTexture(120, 168, 96, 16); // mismo generador de ruido, tinte verde
  gardenTex.repeat.set(8, 24);
  gardenMat = new THREE.MeshStandardMaterial({ map: gardenTex, roughness: 0.88, metalness: 0.0 });
  const garden = new THREE.Mesh(new THREE.PlaneGeometry(42, 260), gardenMat);
  garden.rotation.x = -Math.PI / 2;
  garden.position.set(-23, 0.01, 0); // centro de la franja -44..-2
  garden.receiveShadow = true;
  scene.add(garden);

  // Césped lado tierra adentro (al este de la calle, detrás de las casas hacia el cerro)
  const eastGrass = new THREE.Mesh(new THREE.PlaneGeometry(90, 260), gardenMat);
  eastGrass.rotation.x = -Math.PI / 2;
  eastGrass.position.set(47, 0.01, 0); // centro de la franja 2..92
  eastGrass.receiveShadow = true;
  scene.add(eastGrass);

  // Transición arena→jardín (franja húmeda/oscura)
  ...
  transition.position.set(-44, 0.005, 0);
...
function _buildOcean() {
  const geo = new THREE.PlaneGeometry(200, 400, 80, 120);
  oceanMat = new THREE.ShaderMaterial({
    ...
    vertexShader: `
      ...
      void main() {
        vec3 p = position;
        float h = wave(p.xy, 0.08, 0.9, 0.22) + wave(p.xy * 1.3, 0.13, 1.1, 0.14)
                + wave(p.xy * 0.5, 0.05, 0.6, 0.18) + wave(p.xy * 2.2, 0.22, 1.8, 0.06);
        p.z += h;
        vH = h; vUv = uv;
    ...
    `,
    fragmentShader: `
      uniform vec3 uDeepColor, uShallowColor, uFoamColor, uSunDir;
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const waveX = -69 + i * 1.5; // dentro de la franja de playa, cerca del borde del mar (-70)
  ...
  for (let i = 0; i < 5; i++) {
    ...
    foam.position.set(-68 + i * 3 + Math.random() * 2, 0.05, 0);
...
function _buildBeachDetails() {
  const umbrellaSpots = [[-60, -45], [-55, -20], [-62, 5], [-57, 28], [-60, 48]];
  umbrellaSpots.forEach(([x, z]) => _buildUmbrella(x, z));

  const chairSpots = [[-58, -44], [-53, -19], [-60, 6], [-55, 29]];
...
function _buildPlantaDesalinizadora() {
  ...
  const intake = new THREE.Mesh(new THREE.CylinderGeometry(0.25, 0.25, 25, 10), intakeMat);
  intake.rotation.z = Math.PI / 2;
  intake.position.set(-12.5, 0.4, 0);
  ...
  intakeGrate.position.set(-25, 0.4, 0);
...
function _buildPalmTrees() {
  ...
  for (let z = -50; z <= 50; z += 14) spots.push([-50 + (Math.random() - 0.5) * 6, z]);
  // Cerca del pier
  spots.push([-44, L.PIER.z + 8], [-44, L.PIER.z - 10]);
