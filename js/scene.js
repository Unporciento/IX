...
export function initThree() {
  ...
  // ── Post-procesado: bloom sutil sobre el sol, el tanque, las luces de los
  // postes y los brillos del agua. Umbral alto para que solo lo realmente
  // brillante emita halo — no queremos lavar toda la escena.
...
function _tick() {
  ...
  scene.traverse(obj => {
    ...
    if (obj.userData.isPalmTree) {
      // Balanceo de viento: se mece el árbol completo (tronco + hojas) como
      // una unidad, cada uno con su propia fase/velocidad — más natural que
      // animar cada hoja suelta, y mucho más barato en cómputo.
      const d = obj.userData;
      obj.rotation.x = Math.sin(t * d.windSpeed + d.windPhase) * 0.035;
      obj.rotation.z = Math.cos(t * d.windSpeed * 0.8 + d.windPhase) * 0.035;
    }
    if (obj.userData.isBoat) {
      const d = obj.userData;
      obj.position.y = d.baseY + Math.sin(t * 0.9 + d.bobPhase) * 0.06;
      obj.rotation.z = d.baseRotZ + Math.sin(t * 0.7 + d.bobPhase) * 0.03;
      obj.rotation.x = Math.cos(t * 0.55 + d.bobPhase) * 0.02;
    }
  });
...
function _buildTerrain() {
  ...
  // NOTA: se extiende hasta pasar por debajo de las soleras (curbs en x=±2.45)
  // para que no quede un hueco entre la tierra y la calle — ese hueco era lo
  // que hacía que la carretera se viera "flotando" sobre el terreno.
  const earthFront = new THREE.Mesh(new THREE.PlaneGeometry(29.8, 260), gardenMat);
  earthFront.rotation.x = -Math.PI / 2;
  earthFront.position.set(-17.1, 0.008, 0); // centro de la franja -32..-2.2
  earthFront.receiveShadow = true;
  scene.add(earthFront);

  // Tierra adentro (lado este de la calle, hacia el cerro/planta/parking)
  const earthBack = new THREE.Mesh(new THREE.PlaneGeometry(85.4, 260), gardenMat);
  earthBack.rotation.x = -Math.PI / 2;
  earthBack.position.set(44.7, 0.008, 0); // centro de la franja 2..87
...
function _buildRoads() {
  ...
  // Sombra de contacto bajo cada solera: ancla visualmente la calle al
  // terreno (sin esto, el borde limpio de la solera "flota" ópticamente
  // sobre la tierra, sobre todo con luz rasante).
  const aoMat = new THREE.MeshBasicMaterial({
    color: 0x000000, transparent: true, opacity: 0.28, depthWrite: false,
  });
  [-2.9, 2.9].forEach(ox => {
    const ao = new THREE.Mesh(new THREE.PlaneGeometry(0.9, 96), aoMat);
    ao.rotation.x = -Math.PI / 2;
    ao.position.set(ox, 0.006, 0);
    scene.add(ao);
  });
...
// ─── Sombra de contacto genérica (AO falso) ──────────────────────────────────
// Disco con gradiente radial suave bajo un objeto: el truco más barato para
// que algo apoye visualmente en el suelo en vez de verse "pegado y plano".
let _aoTex = null;
function _getAoTex() {
...
function _contactShadow(x, z, radiusX = 1.4, radiusZ = 1.4, opacity = 0.5) {
  const geo = new THREE.PlaneGeometry(2, 2);
  const mat = new THREE.MeshBasicMaterial({
    map: _getAoTex(), transparent: true, opacity, depthWrite: false,
  });
...
function _buildMountainAndTank() {
  ...
  _contactShadow(mx, mz, 11, 11, 0.35);
...
function _buildCasaPrincipal() {
  ...
  scene.add(house);
  _contactShadow(L.CASA_PRINCIPAL.x, L.CASA_PRINCIPAL.z, 4.6, 4.2, 0.42);
}
...
function _buildSalaDeMaquinas() {
  ...
  scene.add(house);
  _contactShadow(L.SALA_MAQUINAS.x, L.SALA_MAQUINAS.z, 3.4, 3.0, 0.4);
...
function _buildCasitas() {
  L.HOUSE_ROWS_Z.forEach((z, i) => {
    ...
    // Jitter pseudo-aleatorio pero determinístico (misma seed siempre) para
    // que cada cabaña se sienta "puesta a mano" en vez de un grid perfecto —
    // look resort, no maqueta de ingeniería.
    const jw = (_hash(i, 3.1) - 0.5) * 0.8;
    const je = (_hash(i, 7.7) - 0.5) * 0.8;
    const rotW = (_hash(i, 1.4) - 0.5) * 0.09;
    const rotE = (_hash(i, 5.9) - 0.5) * 0.09;
    ...
    west.position.set(L.HOUSE_SIDE_X.west + jw, 0, z + jw * 0.5);
    west.rotation.y = rotW;
    scene.add(west);
    _contactShadow(L.HOUSE_SIDE_X.west + jw, z + jw * 0.5, 2.9, 2.6, 0.4);
    _buildHouseLandscaping(L.HOUSE_SIDE_X.west + jw, z + jw * 0.5, 1, i);
    ...
    east.position.set(L.HOUSE_SIDE_X.east + je, 0, z + je * 0.5);
    east.rotation.y = rotE;
    scene.add(east);
    _contactShadow(L.HOUSE_SIDE_X.east + je, z + je * 0.5, 2.9, 2.6, 0.4);
    _buildHouseLandscaping(L.HOUSE_SIDE_X.east + je, z + je * 0.5, -1, i + 50);
  });
...
// ─── Paisajismo por cabaña: seto bajo + sendero — rompe el look "grid CAD" ──
function _buildHouseLandscaping(x, z, facingSign, seed) {
  const hedgeMat = new THREE.MeshStandardMaterial({ color: 0x3d6b3f, roughness: 0.95 });
  const pathMat  = new THREE.MeshStandardMaterial({ color: 0xd8cdb8, roughness: 0.9 });

  // Seto bajo bordeando el frente del lote, con bultos irregulares
  const hedgeGroup = new THREE.Group();
  const bumps = 5;
  for (let k = 0; k < bumps; k++) {
    const r = 0.35 + _hash(seed, k * 1.3) * 0.18;
    const bump = new THREE.Mesh(new THREE.SphereGeometry(r, 8, 6), hedgeMat);
    bump.position.set((k - (bumps - 1) / 2) * 0.55, r * 0.75, 2.5 * facingSign);
  ...
  // Sendero de piedra entre la puerta y la calle
  const path = new THREE.Mesh(new THREE.PlaneGeometry(1.1, 2.6), pathMat);
  path.rotation.x = -Math.PI / 2;
  path.position.set(x, 0.014, z + 1.2 * facingSign);
  scene.add(path);

  // Una palmera junto a cada segunda cabaña y una reposera en las otras —
  // rompe la simetría del grid y suma vida de "resort" al frente de cada lote.
  if (seed % 2 === 0) {
    _palmTree(x + 2.3 * -facingSign, z - 1.6);
  } else {
    _buildBeachChair(x + 2.2 * -facingSign, z + 1.5);
  }
}
...
function _palmTree(x, z) {
  ...
  group.userData.isPalmTree = true;
  group.userData.windSpeed  = 0.5 + Math.random() * 0.4;
...
// ─── Gaviotas — dan vida al cielo sobre la playa, vuelan en círculos ────────
function _buildBirds() {
  const wingMat = new THREE.MeshBasicMaterial({ color: 0x2b2b2e, side: THREE.DoubleSide });
  const bodyMat = new THREE.MeshBasicMaterial({ color: 0x3a3a3e });
  const count = 6;

  for (let i = 0; i < count; i++) {
    const bird = new THREE.Group();

    const body = new THREE.Mesh(new THREE.ConeGeometry(0.045, 0.32, 6), bodyMat);
    body.rotation.x = Math.PI / 2;
    bird.add(body);

    const wingGeo = new THREE.PlaneGeometry(0.55, 0.14, 1, 1);
    ...
    bird.userData.center    = new THREE.Vector3(-40 + Math.random() * 24, 13 + Math.random() * 9, (Math.random() - 0.5) * 74);
    bird.userData.radiusX   = 16 + Math.random() * 22;
    bird.userData.radiusZ   = 10 + Math.random() * 16;
    bird.userData.orbitSpeed= 0.05 + Math.random() * 0.05;
...
function _updateBirds(t) {
  birdGroup.forEach(b => {
    const d = b.userData;
    const ang     = t * d.orbitSpeed + d.phase;
    const nextAng = ang + 0.02;

    const x = d.center.x + Math.cos(ang) * d.radiusX;
    const z = d.center.z + Math.sin(ang) * d.radiusZ;
    const y = d.center.y + Math.sin(t * 0.3 + d.bobPhase) * 1.4;
    b.position.set(x, y, z);

    const nx = d.center.x + Math.cos(nextAng) * d.radiusX;
    const nz = d.center.z + Math.sin(nextAng) * d.radiusZ;
    b.lookAt(nx, y, nz);

    const flap = Math.sin(t * d.flapSpeed + d.phase) * 0.6;
