...
function _makeGrassTexture() {
  ...
  for (let i = 0; i < size; i++) {
    for (let j = 0; j < size; j++) {
      let v = 0, a = 1, f = 1, m = 0;
      for (let o = 0; o < 4; o++) { v += noise(i/size*16*f, j/size*16*f)*a; m+=a; a*=0.5; f*=2; }
      v /= m;
      const g = 0.7 + v * 0.3;
      const idx = (i * size + j) * 4;
      data[idx]   = Math.floor(78 * g);
      data[idx+1] = Math.floor(145 * g);
      data[idx+2] = Math.floor(68 * g);
      data[idx+3] = 255;
    }
  }
  const tex = new THREE.DataTexture(data, size, size);
...
function _buildTerrain() {
  ...
  const grass = new THREE.Mesh(new THREE.PlaneGeometry(300, 80), grassMat);
  grass.rotation.x = -Math.PI / 2;
  grass.position.set(0, 0.01, 14);
  grass.receiveShadow = true;
  scene.add(grass);

  // Transition strip sand→grass (dark damp strip)
...
function _buildVegetation() {
  // Bushes
  const bushColors = [0x2a6830, 0x3a7838, 0x487035, 0x1f5a28];
  const bushSpots = [
  ...
  // Grass tufts
  const tuftMat = new THREE.MeshStandardMaterial({ color: 0x4a8840, roughness: 0.95, side: THREE.DoubleSide });
  for (let i = 0; i < 60; i++) {
