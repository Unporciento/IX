...
export function initThree() {
  ...
  btnPipe.style.cssText = `position:absolute;bottom:80px;right:20px;padding:10px 18px;
  ...
  buildPalm(-16, 4, 5.5); buildPalm(-16,13, 6.2);
  buildPalm( -7, 9, 6.0); buildPalm(  2, 9, 5.5);
  buildPalm(  9,13, 6.5); buildPalm(-13,14, 5.8);
  buildPalm( 12, 4, 6.0); buildPalm( 12,13, 5.5);
  buildPalm( 14,20, 6.5); buildPalm( -2,20, 5.8);
  buildPalm(-18,-5, 5.0); buildPalm(  9,-5, 5.5);
  ...
  // ── Calle lateral (X=-14), de Z=-18 a Z=20, cada 7 unidades ───────────────
  for (let z = -16; z <= 20; z += 7) {
    buildStreetLamp(-14, z, Math.PI / 2);
  }
  ...
  btnDayNight.style.cssText = `position:absolute;bottom:130px;right:20px;padding:10px 18px;
    background:#1b3d2d;color:#f0ead8;border:2px solid #b8903a;border-radius:6px;
    font-size:13px;font-weight:600;cursor:pointer;letter-spacing:.03em;
    transition:background .2s,color .2s;z-index:100;`;
  ...
  function animate() {
    ...
    pipeOffset = (pipeOffset + delta * 55) % 256;
