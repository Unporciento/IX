...
// Atajos de teclado (G=general, P=plant, N=network, T=top, C=collector,
// S=sector sur, M=casa principal, R=restaurar última vista, Esc=cancelar
// tour/cola de vistas y volver a la vista general)
function _onKeyDown(e) {
  if (e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA') return;

  if (e.key === 'Escape') {
    if (_viewQueue.length || _queueTimer) {
      _viewQueue.length = 0;
      if (_queueTimer) { clearTimeout(_queueTimer); _queueTimer = null; }
      goToView('general');
    }
  ...
  const map = { g: 'general', p: 'plant', n: 'network', t: 'top', c: 'collector', s: 'south', m: 'mainhouse', r: null };
