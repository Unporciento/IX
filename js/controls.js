...
export function initControls(cam, rendererDom) {
  ...
  // Atajos de teclado: 1 = General, 2 = Planta, 3 = Red/Cerro
  window.addEventListener('keydown', e => {
    if (e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA') return;
    if (e.key === '1') goToView('general');
    if (e.key === '2') goToView('plant');
    if (e.key === '3') goToView('network');
    // R = resetear a vista general
    if (e.key === 'r' || e.key === 'R') goToView('general');
  });
}
