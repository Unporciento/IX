...
function _updateSessionStats() {
  // Show stats panel only when there are events (supports both sidebar and injected panel)
  ...
  totalEl.textContent = leakHistory.length;
  waterEl.textContent = totalWater < 1000 ? `${totalWater} L` : `${(totalWater/1000).toFixed(2)} m³`;
  avgEl.textContent   = leakHistory.length ? `${avgTime}s` : '—';
  autoEl.textContent  = autoCount;
}